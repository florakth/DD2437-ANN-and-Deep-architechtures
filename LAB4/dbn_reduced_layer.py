...
class DeepBeliefNet_R():    
    ...
    def recognize(self,true_img,true_lbl):
        ...
        top_v = np.concatenate((p_hid, lbl), axis=1)
        
        for _ in range(self.n_gibbs_recog):
            
            top_v[:,:-n_labels] = p_hid
            _, top_h = self.rbm_stack["hid+lbl--top"].get_h_given_v(top_v)
            _, top_v = self.rbm_stack["hid+lbl--top"].get_v_given_h(top_h)
    ...
    def generate(self,true_lbl,name):
        ...
        top = self.rbm_stack["hid+lbl--top"]
        ...
        for _ in range(self.n_gibbs_gener):
            ...
            vis, _ = hid.get_v_given_h_dir(top_v[:,:-n_labels])
    ...
    def train_greedylayerwise(self, vis_trainset, lbl_trainset, n_iterations):
        ...
        try:
            ...
            self.loadfromfile_rbm(loc="trained_rbm_r",name="vis--hid")
            self.rbm_stack["vis--hid"].untwine_weights()            
            
            self.loadfromfile_rbm(loc="trained_rbm_r",name="hid+lbl--top")        
            ...
            self.savetofile_rbm(loc="trained_rbm_r", name="vis--hid")
            ...
            """
            data_hid_lbl = np.hstack((p_hid, lbl_trainset))

            self.rbm_stack["hid+lbl--top"].cd1(data_hid_lbl, n_iterations)
            self.savetofile_rbm(loc="trained_rbm_r",name="hid+lbl--top")            
    ...
    def train_wakesleep_finetune(self, vis_trainset, lbl_trainset, n_iterations):
        ...
        try:
            self.loadfromfile_dbn(loc="trained_dbn_r", name="vis--hid")

            self.loadfromfile_rbm(loc="trained_dbn_r", name="hid+lbl--top")
            ...
            hidlbl_top = self.rbm_stack["hid+lbl--top"]
            ...
            for it in range(n_iterations):
                ...
                for b_low in range(0, self.n_samples, self.batch_size):
                   ...
                   # wake_p_pen_h, wake_s_pen_h = hid_pen.get_h_given_v_dir(wake_s_hid_h)
                    ...
                    wake_p_top_h, wake_s_top_h = hidlbl_top.get_h_given_v(wake_s_top_v)
                    ...
                    for g_it in range(self.n_gibbs_wakesleep):
                        wake_p_top_v, wake_s_top_v = hidlbl_top.get_v_given_h(wake_s_top_h)
                        wake_p_top_h, wake_s_top_h = hidlbl_top.get_h_given_v(wake_s_top_v)
                   ...
                   # sleep_p_hid_h, sleep_s_hid_h = hid_pen.get_v_given_h_dir(sleep_s_pen_h)
                    ...
                    #gen_p_hid_v, _ = hid_pen.get_v_given_h_dir(wake_s_hid_h)
                    ...
                    hidlbl_top.update_params(wake_s_top_v_0, wake_s_top_h_0, wake_p_top_v, wake_s_top_h)
            ...
            self.savetofile_dbn(loc="trained_dbn_r", name="vis--hid")
            self.savetofile_rbm(loc="trained_dbn_r", name="hid+lbl--top")
